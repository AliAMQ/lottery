/* tslint:disable max-line-length */
import { ComponentFixture, TestBed, fakeAsync, tick } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { Observable, of } from 'rxjs';

import { LotteryTestModule } from '../../../test.module';
import { LotteryUpdateComponent } from 'app/entities/lottery/lottery-update.component';
import { LotteryService } from 'app/entities/lottery/lottery.service';
import { Lottery } from 'app/shared/model/lottery.model';

describe('Component Tests', () => {
    describe('Lottery Management Update Component', () => {
        let comp: LotteryUpdateComponent;
        let fixture: ComponentFixture<LotteryUpdateComponent>;
        let service: LotteryService;

        beforeEach(() => {
            TestBed.configureTestingModule({
                imports: [LotteryTestModule],
                declarations: [LotteryUpdateComponent]
            })
                .overrideTemplate(LotteryUpdateComponent, '')
                .compileComponents();

            fixture = TestBed.createComponent(LotteryUpdateComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(LotteryService);
        });

        describe('save', () => {
            it(
                'Should call update service on save for existing entity',
                fakeAsync(() => {
                    // GIVEN
                    const entity = new Lottery(123);
                    spyOn(service, 'update').and.returnValue(of(new HttpResponse({ body: entity })));
                    comp.lottery = entity;
                    // WHEN
                    comp.save();
                    tick(); // simulate async

                    // THEN
                    expect(service.update).toHaveBeenCalledWith(entity);
                    expect(comp.isSaving).toEqual(false);
                })
            );

            it(
                'Should call create service on save for new entity',
                fakeAsync(() => {
                    // GIVEN
                    const entity = new Lottery();
                    spyOn(service, 'create').and.returnValue(of(new HttpResponse({ body: entity })));
                    comp.lottery = entity;
                    // WHEN
                    comp.save();
                    tick(); // simulate async

                    // THEN
                    expect(service.create).toHaveBeenCalledWith(entity);
                    expect(comp.isSaving).toEqual(false);
                })
            );
        });
    });
});
